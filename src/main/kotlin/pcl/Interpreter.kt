            ...
            when (node) {
                is Node.Identifier -> {
                    ...
                    if (function.stack.size < builtin.arity) {
                        throw BuiltinException(node.range, "Cannot call builtin ${node.name} with ${function.stack.size} items on the stack! (needs at least ${builtin.arity})")
                    }

                    val args = function.stack.pop(builtin.arity)
                    val argTypes = args.map { it::class }
                    val impl = builtin.overloads[argTypes]
                        ?: throw BuiltinException(node.range, "Builtin ${builtin.name} has no overload for arguments of type (${argTypes.map { it.simpleName }.joinToString(", ")})!")
...
fun <T> MutableList<T>.pop(amount: Int = 1) = (0..<amount).map { removeAt(size - 1) }
